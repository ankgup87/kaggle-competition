...
class ComputeNearestMean(CustomMixin):
    ...
    def fit(self, X, y):
        ...
        self.averages = z.groupby(['ModelID', 'saledate_converted']).apply(lambda x: x.tail(1))
...
#        '''col_types is a dict with key types, and value a list columns
...
if __name__=='__main__':
    ...
    cv = -1*(pd.to_datetime(df.saledate) < pd.to_datetime('2011-01-01')).astype(int)
    ...
    test[['SalesID', 'SalePrice']].to_csv('data/solution_benchmark.csv', index=False)
